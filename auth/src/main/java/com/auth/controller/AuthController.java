...
JSONObject jsonResponse = new JSONObject();
	  ...
	  @DeleteMapping("/delete/{id}")
	    ...
	    public ResponseEntity<Map<String, String>> updateEntreprise(
	        ...
	        Map<String, String> response = new HashMap<>();
	        try {
	            // Fetch existing entreprise data
	            EntrepriseDto existingEntreprise = entrepriseService.getEntrepriseById(id);
	            ...
	            if (codetvadocument != null && !codetvadocument.isEmpty()) {
	                ...
	                entrepriseDto.setCodetvadocument(existingEntreprise.getCodetvadocument());
	            }

	            // Handle status document
	            if (status != null && !status.isEmpty()) {
	                if (!status.getContentType().equals("application/pdf")) {
	                    response.put("error", "Document must be a PDF file.");
	                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
	                ...
	                byte[] returnedDoc = status.getBytes();
	                entrepriseDto.setStatus(returnedDoc);
	            } else {
	                entrepriseDto.setStatus(existingEntreprise.getStatus());
	            }
	            ...
	            if (logo != null && !logo.isEmpty()) {
	                try {
	                    byte[] returnedImg = logo.getBytes(); // Converts the logo to a byte array
	                    entrepriseDto.setLogo(returnedImg);
	                } catch (IOException e) {
	                    response.put("error", "Error reading logo file: " + e.getMessage());
	                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
	                }
	            } else {
	                entrepriseDto.setLogo(existingEntreprise.getLogo());
	            }
